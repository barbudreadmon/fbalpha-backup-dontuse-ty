...
UINT8 cpu_readop_arg(INT32 offset)
{
	return i8051_program_data[offset & 0xfff];
}

UINT8 cpu_readop(INT32 offset)
{
	return i8051_program_data[offset & 0xfff];
}

UINT8 program_read_byte_8(INT32 offset)
{
	return i8051_program_data[offset & 0xfff];
}

#else
#define cpu_readop_arg(x)	i8051_program_data[((x)&0xfff)]
#define cpu_readop(x) 		i8051_program_data[((x)&0xfff)]
#define program_read_byte_8(x)	i8051_program_data[((x)&0xfff)]
#endif
...
{
	i8051_write_port = pointer;
}

void i8051_set_read_port_handler(UINT8 (*pointer)(INT32))
{
	i8051_read_port = pointer;
}

UINT8 io_read_byte(INT32 offset)
{
	if (i8051_read_port) {
		return i8051_read_port(offset);
	}

	return 0;
}

void io_write_byte(INT32 offset, UINT8 data)
{
	if (i8051_write_port) {
		i8051_write_port(offset,data);
		return;
	}
}
...
static UINT8 internal_ram_read(INT32 offset);
static void internal_ram_write(INT32 offset, UINT8 data);
static UINT8 internal_ram_iread(INT32 offset);
static UINT32 external_ram_iaddr(INT32 offset, UINT32 mem_mask);
static void internal_ram_iwrite(INT32 offset, UINT8 data);
static UINT8 sfr_read(INT32 offset);
static void sfr_write(INT32 offset, UINT8 data);
static void bit_address_w(INT32 offset, UINT8 data);
static UINT8 bit_address_r(INT32 offset);

#if (HAS_I8052 || HAS_I8752)
static UINT8 i8052_internal_ram_iread(INT32 offset);
static void i8052_internal_ram_iwrite(INT32 offset, UINT8 data);
#endif
//
...
static UINT32 (*hold_eram_iaddr_callback)(INT32 offset, UINT32 mem_mask);
...
void i8051_scan(INT32 nAction)
{
	if (nAction & ACB_DRIVER_DATA) {
		...
		ba.Data	  = &i8051;
		ba.nLen	  = (intptr_t)&i8051.pointer_block_divider - (intptr_t)&i8051;
...
void i8051_set_eram_iaddr_callback(UINT32 (*callback)(INT32 offset, UINT32 mem_mask))
{
	//Hold in static variable since this function can get called before reset has run, which wipes i8051 memory clean
...
UINT8 i8051_internal_r(INT32 offset)
{
...
void i8051_internal_w(INT32 offset, UINT8 data)
{
	//Restrict internal ram to 256 Bytes max
	if(offset < 0x100)
		IRAM_W(offset,data);
...
INLINE void update_timer(int cyc)
{
	...
	if(GET_TR0) {
		...
		switch(mode) {
			...
			case 1:			//16 Bit Timer Mode
				...
				overflow = 0xffff;
				//Check for overflow
				if((UINT32)(count+(cyc/12))>(UINT32)overflow) {
	...
	if(GET_TR2) {
		...
		//Are we in timer mode?
			...
			//Check for overflow
			if((UINT32)(count+timerinc)>(UINT32)overflow) {
...
int	i8752_execute(int cycles)				{ return i8051Run(cycles); }
...
UINT8 i8752_internal_r(INT32 offset)
{
...
void i8752_internal_w(INT32 offset, UINT8 data)
{
