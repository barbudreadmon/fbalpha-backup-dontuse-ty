...
INLINE UINT8 Read##N##_8(t90_Regs *cpustate)    { \
	switch ( cpustate->mode##N )    { \
		...
		default: \
			printf("%04x: unimplemented Read%d_8 mode = %d\n",cpustate->pc.w.l,N,cpustate->mode##N); \
	} \
	return 0; \
} \
INLINE UINT16 Read##N##_16(t90_Regs *cpustate)  { \
	switch ( cpustate->mode##N )    { \
		...
		default: \
			printf("%04x: unimplemented Read%d_16 modes = %d\n",cpustate->pc.w.l,N,cpustate->mode##N); \
	} \
	return 0; \
}
...
INLINE void Write##N##_8( t90_Regs *cpustate, UINT8 value ) { \
	switch ( cpustate->mode##N )    { \
		...
		default: \
			printf("%04x: unimplemented Write%d_8 mode = %d\n",cpustate->pc.w.l,N,cpustate->mode##N); \
	} \
} \
...
{ \
	switch ( cpustate->mode##N )    { \
		...
		default: \
			printf("%04x: unimplemented Write%d_16 mode = %d\n",cpustate->pc.w.l,N,cpustate->mode##N); \
	} \
}
...
void tlcs90SetIRQLine(INT32 line, INT32 state)
{
	// right??
	if (line == 0) line = INT0;
	else if (line == 1) line = INT1;
	else if (line == 2) line = INT2;
	else if (line==0x20)line = INTNMI;

	switch (state)
	{
		case 0: // none
		case 1: // ack
			tlcs90_set_irq_line(line, state);
		break;

		case 2: // auto
			tlcs90_set_irq_line(line, 1);
			tlcs90Run(10);
			tlcs90_set_irq_line(line, 0);
			tlcs90Run(10);
		break;
	}
}
...
void tlcs90NewFrame()
{
	...
	cpustate->total_cycles = cpustate->nCycles = cpustate->icount = 0;

//	bprintf (0, _T("NewFrame\n"));
}
...
UINT8 t90_internal_registers_r(UINT16 offset)
{
	t90_Regs *cpustate =  &tlcs90_data[0];

	#define RIO     tlcs90_io_read_byte( T90_IOBASE+offset )
...
void t90_internal_registers_w(UINT16 offset, UINT8 data)
{
	#define WIO     tlcs90_io_write_byte( T90_IOBASE+offset, data )
